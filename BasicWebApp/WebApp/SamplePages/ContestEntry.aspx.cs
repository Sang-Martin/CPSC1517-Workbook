...
namespace WebApp.SamplePages
{
    ...
    {
        //due to the fact that this example is NOT using a database to retain data
        //  the following List<T> will serve as the page collection dataset
        //the List<T> is declared as static
        //the List<T> will remain active as the web application is running
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //check your Page.IsPostBack to see if this is the 1st presentation of the page (IsPostBack = false)
            if (!Page.IsPostBack)
            {
        ...
        {
            //to execute validation controls (not HTML5) on the server side
            if(Page.IsValid)
            {
                if(Terms.Checked)
                {
                    ...
                    //record my entry
                    //colecct all the entries and display them
                    ...
                    //use a collection display control that displays multiple separate columns: GridView
                    //requirements:
        ...
        {
            //Textbox
            FirstName.Text = LastName.Text = StreetAddress1.Text =
            StreetAddress2.Text = City.Text = PostalCode.Text =
            EmailAddress.Text = CheckAnswer.Text = "";
